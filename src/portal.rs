...
//! this module is only how a portal looks. Twelve baked frames from
...
use crate::{Position, PORTAL_FRAMES, PORTAL_ICON_COL, PORTAL_ICON_SIZE, PORTAL_TEXTURE_SIZE};
...
/// Which of the twelve frames a portal centred at `center` shows at `time`:
...
pub fn portal_source_rec(frame: i32) -> Rectangle {
    Rectangle::new(frame as f32 * PORTAL_TEXTURE_SIZE, 0.0, PORTAL_TEXTURE_SIZE, PORTAL_TEXTURE_SIZE)
}

/// Source rectangle of the 32px builder icon in the sheet's last cell.
pub fn portal_icon_source_rec() -> Rectangle {
    Rectangle::new(PORTAL_ICON_COL as f32 * PORTAL_TEXTURE_SIZE, 0.0, PORTAL_ICON_SIZE, PORTAL_ICON_SIZE)
}
...
mod tests {
    ...
    #[test]
    fn frames_cycle_and_the_icon_sits_in_the_last_cell() {
        let at = Position::new(320.0, 160.0);
        let f0 = portal_frame(at, 0.0);
        assert!((0..PORTAL_FRAMES).contains(&f0));
        // A whole period later the same frame shows again; a twelfth of one
        // later the next frame does.
        ...
        assert_eq!((icon.x, icon.y, icon.width, icon.height), (PORTAL_ICON_COL as f32 * PORTAL_TEXTURE_SIZE, 0.0, 32.0, 32.0));
