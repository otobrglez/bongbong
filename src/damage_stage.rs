...
impl DamageStage {
    ...
    pub fn from_damage(damage: f32) -> Option<DamageStage> {
        ...
        if damage <= 0.0 {
            ...
            Some(DamageStage { frames: &[0], fps: 0.0 }) // dusty
        } else if damage <= 30.0 {
            Some(DamageStage { frames: &[1], fps: 0.0 }) // gray
        } else if damage <= 45.0 {
            Some(DamageStage { frames: &[2, 3], fps: 5.0 }) // small smoke
        } else if damage <= 60.0 {
            Some(DamageStage { frames: &[4, 5], fps: 5.0 }) // more smoke
        } else if damage <= 75.0 {
            Some(DamageStage { frames: &[6, 7], fps: 8.0 }) // small fire
        } else if damage < MAX_DAMAGE {
            Some(DamageStage { frames: &[8, 9], fps: 10.0 }) // large fire
        } else {
            Some(DamageStage { frames: &[10, 11, 12], fps: 8.0 }) // wrecked
        }
    }
