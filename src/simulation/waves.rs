...
use super::{lay_tracks, roll_enemy_tank, with_frog, with_tank, with_tank_mut, Event, Frame, Game};
...
impl Game {
    ...
    pub(super) fn rollin_phase(&mut self, f: &mut Frame) {
        ...
        for (entity, slot) in arrived {
            ...
            self.world.insert_one(entity, Ai::default()).expect("entity from this frame's roll-in query still exists");
    ...
    fn spawn_in_band(&mut self, f: &mut Frame, row: i32) {
        ...
        self.world.spawn((tank, Ai::default()));
        f.events.push(Event::TankEntered { slot });
    }
