...
1. **Retinting the ground tileset** (`tools/retint_ground.py`, idempotent —
   ...
   *own* deeper tree-canopy green (`#85A643` → `#619541`, next to its
   ...
   `#B1A567`); wood/red/teal/water/greys untouched. That curve is now the
   script's `meadow` theme; the live default is `desert` (pale dust,
   packed-earth roads), selected with `BONGBONG_THEME`. Details in
