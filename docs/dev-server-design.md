...
| `builder_tool` | `tool` | selects a brush by name (the 22 cell tools or `eraser`) through the category's own path; without `tool`, reports the active tool and every category's current tool and list |
...
| `builder_settings` | `tanks`, `tank`, `mission`, `spawn`, `waves`, `wave_size`, `wave_growth`, `tier_start`, `tier_end`, `reset` | the map's level keys: an absent field is untouched, `null` means auto (mission/spawn fall back to protect/band); each changed field is one undo step, `reset: true` then reverts cells and settings to the baseline. Replies with the current values and `cli_overrides` (which fields `Game`'s CLI/`restart` values outrank at PLAY) |
...
| `click` | `x`, `y`, `button`, `drag_to` | a raw press at a window position on the same hit-tests the mouse gets: in play, the BUILD button, the dialog's buttons (outside the panel = keep playing) or a field `tap`; in build, the bar's buttons (PLAY starts the round), a dropdown row, a stepper, a field cell; `drag_to` drags in 8 px steps and releases. Replies like `mode` plus `tap` |
| `key` | `key` (tab\|escape\|enter\|undo\|redo\|backspace), `text` | one key for one frame: in play, Tab opens/closes the dialog, Esc keeps playing, Enter leaves; in build, Tab is PLAY and the rest go through `BuilderInput` (`text` types into an open prompt). Replies like `mode` |
