...
- **The live `punyworld-overworld-tileset.png` here is a retinted copy**,
  ...
  `_original/punyworld-overworld-tileset.png`, in the theme `BONGBONG_THEME`
  selects: `desert` (the default — pale dust, packed-earth roads, hardpan
  drifts) or `meadow` (deeper grass, muted dirt); see `docs/GROUND_SPEC.md`
  §1. `_original/` is the untouched third-party art; keep it pristine and
  rerun the script to regenerate the live copy.
