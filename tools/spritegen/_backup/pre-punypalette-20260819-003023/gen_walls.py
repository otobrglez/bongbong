...
# bright sky-blue for glass instead of the old washed-out pale blue. Walls
# are the backdrop, so they stay a notch less saturated/lighter in value
# than the tank roster's primary/secondary hues (see gen_tanks.py's roster
# comment) rather than matching it outright -- but drawn from the palette's
# warm mid-tones, not its dim/grey corner, so terrain doesn't read as drab
# concrete next to candy-bright tanks.
SCORCH = (0x4C, 0x3E, 0x24, 255)
DUST = (0xE6, 0x90, 0x4E, 255)
EMBER = (0xCD, 0x68, 0x3D, 255)
FIRE_D = (0xEA, 0x4F, 0x36, 255)
FIRE_M = (0xF7, 0x96, 0x17, 255)
FIRE_C = (0xFB, 0xFF, 0x86, 255)

GL_D = (0x4D, 0x9B, 0xE6, 255)
GL_M = (0x8F, 0xD3, 0xFF, 255)
...
# ======================================================================
# One shared R64 tone for all four bond patterns -- per the original design
...
# snap) gives natural per-brick variation on top. Warm amber rather than the
# first pass's dusty rose-brick, for a friendlier "gingerbread" warmth; the
# mortar is a colourful rust-brown (not grey) but still darker than the
# brick itself so grout lines stay readable.
BRICK_MORTAR = (0x9E, 0x45, 0x39, 255)
BRICKS = [
    dict(name='running', base=(0xE6, 0x90, 0x4E), pw=8,  ph=4, bond='run'),
    dict(name='block',   base=(0xE6, 0x90, 0x4E), pw=16, ph=8, bond='run'),
    dict(name='long',    base=(0xE6, 0x90, 0x4E), pw=16, ph=4, bond='run'),
    dict(name='stacked', base=(0xE6, 0x90, 0x4E), pw=8,  ph=8, bond='stack'),
]
...
# differing by surface treatment" (per WALLS_SPEC.md). Pale steel-blue, a
# couple of steps brighter than the first pass's dark steel-grey, so it
# reads as shiny metal rather than dull concrete.
IRONS = [
    dict(name='riveted',    base=(0x9B, 0xAB, 0xB2), style='rivet'),
    dict(name='corrugated', base=(0x9B, 0xAB, 0xB2), style='corr'),
    dict(name='banded',     base=(0x9B, 0xAB, 0xB2), style='band'),
    dict(name='tread',      base=(0x9B, 0xAB, 0xB2), style='tread'),
]
...
# WOOD
# ======================================================================
# Warm honey-gold -- brighter than the first pass's muted tan, and picked
# distinctly from BRICKS' amber so wood still reads as a different material
# from brick at a glance, not just a different bond pattern.
WOODS = [
    dict(name='planks_h', base=(0xFB, 0xB9, 0x54), style='horiz'),
    dict(name='planks_v', base=(0xFB, 0xB9, 0x54), style='vert'),
    dict(name='stagger',  base=(0xFB, 0xB9, 0x54), style='stag'),
    dict(name='palisade', base=(0xFB, 0xB9, 0x54), style='logs'),
]
...
# pixels into new off-palette colours, drifting off the Resurrect 64 set
# every sheet here snaps to, see resurrect64.py) so each 2x2 block collapses
