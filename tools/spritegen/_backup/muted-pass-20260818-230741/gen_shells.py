...
from resurrect64 import GREY_DK, GREY_LT

S = 32
OUT = os.environ.get('SPRITE_OUT', 'assets/sprites')
os.makedirs(OUT, exist_ok=True)

# ---- palettes, curated picks from RESURRECT64 (each family keeps its old
# dark/mid/core/smoke roles, drawn from the palette's own red/orange/blue
# ramps -- the blue family in particular lands almost exactly on R64's sky-
# blue ramp with a punchy white spark core instead of the old pale-blue one).
# (smoke_rgb, smoke_a_dense, smoke_a_light, dark, mid, core)
FAMILIES = {
    'orange': dict(smoke=(0x7F, 0x70, 0x8A), sa=(220, 110),
                   D=(0xEA, 0x4F, 0x36), M=(0xF7, 0x96, 0x17), C=(0xFB, 0xFF, 0x86)),
    'red':    dict(smoke=(0x3E, 0x35, 0x46), sa=(230, 120),
                   D=(0xAE, 0x23, 0x34), M=(0xFB, 0x6B, 0x1D), C=(0xFB, 0xB9, 0x54)),
    'blue':   dict(smoke=(0x9B, 0xAB, 0xB2), sa=(200, 100),
                   D=(0x4D, 0x9B, 0xE6), M=(0x8F, 0xD3, 0xFF), C=(0xFF, 0xFF, 0xFF)),
}

# shared casing colors (identical across all rows in the original)
CASE_DK = GREY_DK + (255,)
CASE_HI = GREY_LT + (255,)
NOSE = (0xE6, 0x90, 0x4E, 255)  # R64 burnished bronze-gold
